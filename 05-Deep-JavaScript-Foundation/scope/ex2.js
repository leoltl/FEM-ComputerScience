...
function paidStudentsToEnroll() {
  ...
  const paidButNotEnrolledStudentIDs = studentRecords.filter(r => r.paid)
  .filter(r => !currentEnrollment.includes(r.id))
  .map(r => r.id)
	return [...currentEnrollment, ...paidButNotEnrolledStudentIDs]
}

function remindUnpaid(recordIds) {
	printRecords(recordIds.map(findRecond).filter(r => !r.paid).map(r => r.id))
}
